...
4)	Merge the datasets from 2) via row and column bind and stores in a new data frame.  
...
7)	Then, substitute the column names with more significant names instead of the abbreviated form provided in the raw dataset. Additional formatting such as elimination of "-",")" and "(" without quotes in variable. The "activity" column was also converted from character to factor. Step 6) is done basically to conform to the principles of tidy data.  
8)	Using the cleaned data in 6), we then compute the average for each variable for each subject and activity which was achieved via the aggregate command. The result of this computation is saved as "Tidy_Data.txt" without quotes in the current working directory.  
